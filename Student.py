...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 12 16:25:28 2021
...
#create distiller
# Create the student
def create_Student(x_train, y_train, x_test, y_test, input_shape, nb_classes, output_directory, filters, filters2):
  recupereTeacherLossAccurayTest2=[]

#  teacher_recupere_model = output_directory.replace('Student', 'teacher')
  ...
  for i in range (6):
    ...
    #teacher = keras.models.load_model(f"best_model.hdf5")
    ...
    history2= distiller.fit(x_train, y_train,batch_size=mini_batch_size, epochs=1,validation_data=(x_test,y_test),
          callbacks=callbacks,)
    ...
    #plt.title(f"best_model_distiller{i}.h5")
    #plt.ylabel('')
    ...
    plt.title(f"{output_directory}/best_model_distiller{i}.h5")
    plt.xlabel("epoch")
    plt.ylabel('')
    plt.legend()
    plt.savefig(output_directory + '/figdistfold'+ str(i)+'.png')
    plt.show()
    plt.close()
